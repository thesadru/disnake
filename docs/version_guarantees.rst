...
- ``major`` bumps indicate a significant refactoring or other large changes that wold constitute such an increase (will most likely include breaking changes)
...
If it's not listed in the documentation here then it is not part of the public API and is thus bond to change.
This includes attributes that start with an underscore or functions withot an underscore that are not documented.
...
- Renaming a function withot an alias to an old function.
