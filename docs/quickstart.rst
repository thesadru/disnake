...
This page gives a brief introduction to the library. It assumes yo have the library installed,
if yo don't check the :ref:`installing` portion.

A Minimal Bot
---------------

Let's make a bot that responds to a specific message and walk yo throgh it.
...
.. code-block:: python3
    ...
    client.run('yor token here')

Let's name this file ``example_bot.py``. Make sure not to name it ``disnake`` as that'll conflict
with the library.

There's a lot going on here, so let's walk yo throgh it step by step.

1. The first line just imports the library, if this raises a `ModuleNotFondError` or `ImportError`
   then head on over to :ref:`installing` section to properly install.
2. Next, we create an instance of a :class:`Client`. This client is or connection to Discord.
3. We then use the :meth:`Client.event` decorator to register an event. This library has many events.
   Since this library is asynchronos, we do things in a "callback" style manner.

   A callback is essentially a function that is called when something happens. In or case,
...
4. Since the :func:`on_message` event triggers for *every* message received, we have to make
   sure that we ignore messages from orselves. We do this by checking if the :attr:`Message.author`
...
6. Finally, we run the bot with or login token. If yo need help getting yor token or creating a bot,
...
Now yo can try playing arond with yor basic bot.
